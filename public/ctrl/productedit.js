var app = angular.module('myApp', ['frapontillo.bootstrap-duallistbox']);
app.config(config);
...
app.controller('productCtrl', function ($scope, $http) {
    ...
    $scope.submitForm = function (isValid) {
        ...
        if (isValid) {
            ...
            var req = {
                method: 'POST',
                url: '/textla/product/post-update',
                headers: {
    ...
    $scope.init = function () {
        if (window.location.search.indexOf("=") != -1) {
            ...
            var req = {
                method: 'GET',
                url: '/textla/product/detail?id=' + id,
                headers: {
                    'X-CSRF-Token': $('[name="_token"]').val()
                }
            }
            $http(req).then(function successCallback(response) {
                if (response.data) {
                    ...
                    for (const item of $scope.product.product_options) {
                        item.options_values = JSON.parse(item.options_values);
                    }
                    $scope.allProducts = response.data.allProduct;
                    $scope.allCategories = response.data.allCategories;
                    console.log($scope.product)
                }
            }, function errorCallback(response) {
                console.log(response);
